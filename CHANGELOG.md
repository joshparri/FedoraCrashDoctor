...
## Unreleased
- Added `incident_model.py`: each incident `collector.build_incidents()` produces now gets a stable, content-derived `incident_id` (so recomputing the same incident on a rescan yields the same id, not a fresh one) and a `classification` answering "WHAT actually froze?" -- one of a fixed set of incident classes (application-only crash, plasmashell/panel failure, KWin/compositor hang, GPU/DRM/i915, memory thrashing, OOM kill contained, storage/filesystem blocking, kernel hard lockup, hardware/RAS, clean reboot, or `insufficient_evidence`) with the reasoning that led there, plus `evidence_sources`/`evidence_completeness` so a caller can tell "not checked" apart from "checked and healthy". A bare unclean shutdown with no corroborating symptom evidence is deliberately classified `insufficient_evidence` rather than guessed as a specific cause -- this is a first, tested increment of the structured Incident model; joining canary/heartbeat telemetry and user-reported observations, fingerprinting/recurrence across boots, and a wizard for capturing "what did you see?" are not yet implemented.
