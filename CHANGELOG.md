...
## Unreleased
- Wired Sections 5-9's Incident model into the real application instead of only proving out via direct test calls. `collector.collect()` now calls `confidence_explanation.explain_incident()` for every incident on every real scan (CLI and GUI alike, since both go through `collect()`), attaching `hypothesis_explanations`. Added `incident_history.py`, which reads previously saved `scan-*.json` reports (bounded: at most the 200 most recent files, any file over 20 MiB skipped) to build real cross-scan `incident_fingerprint` families -- this deliberately runs only in the unprivileged GUI process (`MainWindow._attach_incident_recurrence()`, called from `scan_finished()` before a scan is saved), never inside `privileged_helper.py`, so recurrence tracking adds no new privileged attack surface. Once a failure pattern has recurred, its explanation's `recurrence_context` is upgraded in place and the report gains an `incident_families` list (patterns seen more than once). A broken or oversized history file never fails the scan it's enriching. Verified with real MainWindow instances against a redirected (never the real `~/Documents`) report directory, including that a scan already saved to disk isn't double-counted against itself.
